...
public class SocialMentionController {
    ...
    public String analyze(@Body SocialMention socialMention) {
        ...
        if (analyzer.isPresent()){
            return analyzer.get().analyze(socialMediaData).getMessage();
        }
        ...
        if (isFacebook || isTweeter) {
            ...
            if (socialMention.getMessage().contains("comments:")) {
                facebookCommentsScore = FacebookAnalyzer.calculateFacebookCommentsScore(socialMention.getMessage().substring(socialMention.getMessage().indexOf("comments:")));
...
// Analyze facebook post (if facebook is already low then skip this analysis)
            ...
            if (isFacebook && facebookScore > -100) {
                ...
                dbService.insertFBPost(ANALYZED_FB_TABLE, facebookScore, socialMention.getMessage(), socialMention.getFacebookAccount());
            }
